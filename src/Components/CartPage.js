...
const [zip, setZip] = useState('');
  ...
  const handleRemove = async (itemId) => {
    try {
      ...
      toast.error('Wystąpił błąd podczas usuwania przedmiotu z koszyka.');
    }
  };

  // Obliczanie sumy częściowej i całkowitej
  const deliveryCost = 8.0; // Koszt dostawy
  ...
  const handleSubmit = async (event) => {
	...
	// Sprawdzenie, czy wszystkie pola zostały wypełnione (zakładając, że masz odpowiednie stany ustawione)
	if (!email || !cardHolder || !cardNumber || !expiryDate || !cvc || !address || !city || !zip) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseData.message || 'Błąd podczas składania zamówienia',
        );
      }
      ...
      toast.success('Zamówienie złóżone pomyślnie');
    } catch (error) {
      console.error('Błąd:', error);
      ...
      <div className="flex-grow grid pt-7 pb-7 sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32 background-component">
        <div className="px-4 pt-8">
          ...
          <p className="text-white">
            Sprawdź swoje przedmioty. I wybierz odpowiednią metodę wysyłki.
          </p>
              ...
              >
                ...
                <div className="flex w-full flex-col px-4 py-4">
                  ...
                  <button
                    ...
                    className="self-end rounded  px-2 py-1 cart-button"
                  >
                    Usuń
                  </button>
                </div>
        ...
        <div className="mt-10 bg-gray-50 px-4 pt-8 lg:mt-0">
          ...
          <p class="text-gray-400">
            Dokończ zamówienie, podając szczegóły płatności.
          </p>
          <form onSubmit={handleSubmit} className="mt-4">
            ...
            <div class="flex">
              ...
              <input
                type="text"
                name="credit-expiry"
                class="w-full rounded-md border border-gray-200 px-2 py-3 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500 mx-2"
                placeholder="MM/YY"
				required 
              />
              <input
                type="text"
                name="credit-cvc"
                class="w-1/6 flex-shrink-0 rounded-md border border-gray-200 px-2 py-3 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500 mx-2"
                placeholder="CVC"
				required 
              />
            </div>
            ...
            <div class="flex flex-col sm:flex-row">
              ...
              <input
                type="text"
                name="credit-expiry"
                class="w-full rounded-md border border-gray-200 px-2 py-3 mx-2 text-sm shadow-sm outline-none focus:z-10 focus:border-blue-500 focus:ring-blue-500"
                placeholder="Miasto"
				required 
              />
              ...
              <input
                type="text"
                name="billing-zip"
                class="flex-shrink-0 rounded-md border border-gray-200 px-4 py-3 mx-2 text-sm shadow-sm outline-none sm:w-1/6 focus:z-10 focus:border-blue-500 focus:ring-blue-500"
                placeholder="Kod"
				required 
              />
            </div>
          ...
          <button
		   type="submit"
            ...
            class="mt-4 mb-8 w-full rounded-md px-6 py-3 font-medium cart-button"
          >
            Złóż zamówienie
          </button></form>
        </div>
